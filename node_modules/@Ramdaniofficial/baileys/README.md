...
Baileys now supports the latest multi-device beta. However, as it's a completely different implementation from WA Web & will break a lot of things, it's on its own branch which will be merged into master once mutli-device hits production. You may find the MD repo [here](https://github.com/Ramdaniofficial/baileys/tree/multi-device).
...
1. stable: `npm install @Ramdaniofficial/baileys`
2. stabl-ish w quicker fixes & latest features: `npm install github:Ramdaniofficial/baileys` 

Do note, the library will likely vary if you're using the NPM package, read that [here](https://www.npmjs.com/package/@Ramdaniofficial/baileys)

Then import in your code using:
``` ts 
import { WAConnection } from '@Ramdaniofficial/baileys'
```
...
Do note, the `conn.chats` object is a [KeyedDB](https://github.com/Ramdaniofficial/keyed-db). This is done for the following reasons:
...
``` ts
import { WAConnection, ProxyAgent } from '@Ramdaniofficial/baileys'
...
``` ts
import { Presence } from '@Ramdaniofficial/baileys'
...
``` ts
import { MessageType } from '@Ramdaniofficial/baileys'
...
- To change group settings
    ``` ts
    import { GroupSettingChange } from '@Ramdaniofficial/baileys'
